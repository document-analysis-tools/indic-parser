...
class Predict:
    ...
    def _column_to_dataframe(column, image):
        ...
        crop = crop + white
        ocr = image_to_string(Image.fromarray(crop.astype(np.uint8)), lang='san_iitb') 
        return pd.DataFrame({"col": [value for value in ocr.split("\n") if len(value) > 0]})


@click.command()
@click.option('--image_path', default="sample.jpg")
@click.option('--model_weights', default="best_model.ckpt")
def predict(image_path: str, model_weights: str) -> List[pd.DataFrame]:
    ...
    f.write(str(pred.predict(image)))
    f.close()
